...
export default function SearchOverlay({ isOpen, onClose }: SearchOverlayProps) {
  ...
  const handleSearch = () => {
    if (query.trim()) {
      if (activeSuggestionIndex >= 0 && suggestions[activeSuggestionIndex] && isAutocompleteMode) {
        ...
        // const searchResults = await searchProducts(suggestions[activeSuggestionIndex]);
        // navigate('/products', { state: { searchResults, query: suggestions[activeSuggestionIndex] } });
        navigate(`/products?q=${encodeURIComponent(suggestions[activeSuggestionIndex])}`);
      } else {
        // Navigate with current query
        // Actual API implementation would be:
        // const searchResults = await searchProducts(query);
        // navigate('/products', { state: { searchResults, query } });
  ...
  const handleSuggestionClick = (suggestion: string) => {
    // Actual API implementation would be:
    // const searchResults = await searchProducts(suggestion);
    // navigate('/products', { state: { searchResults, query: suggestion } });
